...
- `maddev-lang` (CLI/compiler)
- `maddev-ts-plugin` (TS Language Service plugin for IntelliSense)

Steps:
1. Build before publishing (already wired via `prepare`): `npm run build`.
2. Publish the plugin first:
   ```bash
   cd ts-plugin
   npm publish --access public
   cd ..
   ```
3. Publish the compiler/CLI:
   ```bash
   npm publish --access public
   ```
...
```
Then in your project `tsconfig.json`:
```json
{
  "compilerOptions": {
    "module": "NodeNext",
...
```
Use `maddevc` from `node_modules/.bin` to transpile `.mts` files, and the plugin will give IntelliSense for the custom keywords.
